...
(async () => {
  ...
  try {
    latestVersion = await lv("zoos-mp-cli");
  } catch (error) {
    pauseSpinner();
  ...
  cli.command("create <projectName>", "创建一个新的小程序项目").action(async (projectName) => {
    logWithSpinner("从仓库下载小程序项目模板： https://github.com/xxiaojiee/zoos-mp-template.git ");
    const sao = require("sao");
    const app = sao({
      generator: "direct:https://github.com/xxiaojiee/zoos-mp-template.git",
      clone: true,
      update: true,
      outDir: projectName,
    });
